...
time comm -12 $REDUX_ENV/hg38.0 $REDUX_ENV/new.1 > $REDUX_ENV/new.2
...
time cat $REDUX_ENV/new.1 | sed "/\.$/d" > $REDUX_ENV/new.3
#cnt: 343046

echo;echo "step 6 - old.1 - from the 'new' list, these are the ones exactly seen in hg38"
echo "$ comm -12 hg38.0 new.3 > old.1"
time comm -12 $REDUX_ENV/hg38.0 $REDUX_ENV/new.3 |awk '{print $1}' | sort -u> $REDUX_ENV/old.1
#cnt: 342317
...
time cat $REDUX_ENV/old.1|cut -d',' -f1 | sort -u > $REDUX_ENV/old.2
...
time cat $REDUX_ENV/new.3|cut -d',' -f1 | awk '{print $1}' | sort -u > $REDUX_ENV/new.4

echo;echo "step 10 - new.5 - now let's use comm to exclude old.2 references"
echo "$ comm -23 old.2 new.4 |sed -e 's/$/,/g' > new.5"
time comm -13 $REDUX_ENV/old.2 $REDUX_ENV/new.4 > $REDUX_ENV/new.5
...
time comm -12 $REDUX_ENV/hg38.1 $REDUX_ENV/new.5 |sed -e 's/$/,/g' |sed -e 's/ //g' > $REDUX_ENV/new.6
#hg38.1: 10147234,G,T
#new.5.: 10147234,G,A <-- truly new

echo;echo "step 12 - new.7 - and here's where we get the rest of the data for that new data"
echo "$ grep -f new.6 hg38.0 > new.7"
time grep -f $REDUX_ENV/new.6 $REDUX_ENV/hg38.0 > $REDUX_ENV/new.7

echo;echo "step 13 - get count of unique new variants"
cntWInsDel=`cat $REDUX_ENV/new.7 | wc -l`
cntWOInsDel=`cat $REDUX_ENV/new.7 | sed '/ins/d' |wc -l`
