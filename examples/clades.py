...
buildarray=dendropy = create = stats1 = stats2 = docalls = listfiles = listbed = mergeup = updatesnps = False
...
def variant_array():
    ...
    ppl = []
    c1 = c1.execute('select id from files where kit=1 order by seq,name')
    for (p,) in c1:
        ppl.append(p)
    # build a vector of variants
    var = []
    c1 = c1.execute('select distinct v.id from variants v, vcfcalls c where c.vid=v.id')
    # build a 2d array
    arr = {}
    for pp in ppl:
        arr[pp] = defaultdict()
    ...
    for (v,) in c1:
        var.append(v)
    c1 = c1.execute('select p.id, v.id from files p, variants v, vcfcalls c where p.id=c.id and c.vid=v.id and p.kit=1')
    for (p,v) in c1:
        arr[p][v] = 1
    return arr, ppl, var
                        ...
                        if filt == 'REJECTED':
                            ...
                            if ref != '.' and alt != '.':
                                ...
                                if len(ref) == 1 and len(alt) == 1:
                                    ...
                                    ni += 1 # indels
                trace(2,'{}: {} variants; {} SNPs; {} indels'.format(fname, nv,ns,ni))
...
# create fasta definitions for dendropy
if dendropy:
    import dendropy
    ...
    v_vector = []
    dd = {}
    c1 = c1.execute('select distinct v.pos from vcfcalls c, variants v where v.id=c.vid and c.origin="k" order by v.pos')
    c1 = c1.execute('select v.pos,count(c.vid) from vcfcalls c, variants v where v.id=c.vid and c.origin="k" group by 1 order by 2 desc,1')
    for (pos,count) in c1:
        if count < 2700 and count > 1:
            v_vector.append(pos)
    v_vector.sort()
    trace (1, 'dendropy: number of calls is {}'.format(len(v_vector)))

    c1 = c1.execute('select id,name from files where kit=1')
    for (pid,name) in c1:
        ranges = c2.execute('select minaddr,maxaddr from bed where id=? order by minaddr', (pid,))
        kit_ranges = []
        for minaddr,maxaddr in ranges:
            kit_ranges.append((minaddr,maxaddr))
        coverage_vector = in_range(v_vector, kit_ranges)
        c2 = c2.execute('select v.pos,v.ref,v.alt from vcfcalls c, variants v where c.id=? and c.vid=v.id order by v.pos', (pid,))
        ii = 0
        out = ''
        count = 0
        for (pos,ref,alt) in c2:
            count += 1
            while pos > v_vector[ii]:
                out += '?' # this kit doesn't have the call
                ii += 1
            if len(alt) != 1 or len(ref) != 1:
                out += '?' # pass on indels for now
            elif alt != '.':
                out += alt # had a good call SNP
            else:
                out += ref # has the reference value
            ii += 1
        dd[pid] = out
        trace(2, '{}:{}'.format (name, out))
        trace(1, 'dendropy: done with {} ({} calls)'.format(pid,count))
    dna = dendropy.DnaCharacterMatrix.from_dict(dd)
    trace(1, 'dendropy: built DnaCharacterMatrix')
    tree = paup.estimate_tree(dna,tree_est_criterion='nj')
    print (tree.as_string(schema="newick"))
    # https://pythonhosted.org/DendroPy/primer/paup.html
    # http://www.paup.sc.fsu.edu/unix/index.html
    example = """
>>> dd = {'jef':'AGCTACG', 'fred':'AGCGAGG', 'barney':'AGCTAGG', 'mike':'TGCTAGG'}
>>> dna = dendropy.DnaCharacterMatrix.from_dict(dd)
>>> tree = paup.estimate_tree(dna, tree_est_criterion='nj')
>>> print (tree.as_string(schema="newick"))
[&R] (jef:1.0,fred:1.0,(barney:0.0,mike:1.0):0.0);
"""
...
if buildarray:
    trace(1, 'build array at {:.2f} seconds'.format(time.time() - t0))
    arr, ppl, var = variant_array()
    trace(1, 'built at {:.2f} seconds'.format(time.time() - t0))
    trace(1, '{} people, {} variants'.format(len(ppl),len(var)))
    trace(2,'people: {}'.format(ppl))
    trace(2,'variants: {}'.format(var))
    trace(3,'person,variant:')
    for pp in ppl:
        pd = arr[pp]
        for vv in pd:
            try:
                x = pd[vv]
                if x:
                    trace(3, '{},{}'.format(pp,vv))
            except KeyError:
                pass
            except:
                raise
    trace(1, 'array walked at {:.2f} seconds'.format(time.time() - t0))
