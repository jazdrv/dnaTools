...
#      zip redux.zip redux.bash positives-and-no-calls.py snps_hg19.csv age.bed poisson.tbl cpoisson.tbl implications.txt badlist.txt recurrencies.txt cladenames.txt events.svg treefoot.html merge-ignore.txt
...
awk 'NR<16 || $1+0==0 || $2~"Z381" || $16<589' FS=, short-report.csv | cut -d, -f1-588 > short-report-z301.csv
awk 'NR<16 || $1+0==0 || $2~"Z381" || $16>=589' FS=, short-report.csv | cut -d, -f1-17,589- > short-report-xz301.csv
...
intable==1 && $0~"<td bg" {split($11,x,"="); gsub(/"/,"",x[2]); if (x[2]+0<m) print}' m=572 report.html > z301.html
...
intable==1 && $0~"</td>" && n==1 {print; n=0} \
intable==1 && substr($0,1,2)=="<!" && $5>=m {cols-=$6; s+=$6; print $0, "<!",cols,">"} \
intable==1 && $0~"<td bg" {split($11,x,"="); gsub(/"/,"",x[2]); if (x[2]+0>=m) print}' m=572 report.html > xz301.html
